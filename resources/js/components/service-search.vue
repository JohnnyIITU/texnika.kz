                    ...
                    <div class="row ml-0 mr-0 align-items-stretch">
                        ...
                        <div class="col-lg-3 pl-0 pr-0 position-static">
                            <div class="dropdown h-100">
                                <a href="#" class="filter-button" role="button" id="dropdownStatusLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Тип обслуживания<span>{{serviceTypeLabel}}</span></a>
                                <div class="dropdown-menu dropdown-menu--filter" aria-labelledby="dropdownStatusLink">
                                    <ul class="filter-list">
                                        <li class="filter-list__item" :class="item.class" v-for="item in serviceTypeOptions" @click="changeServiceTypeKey(item.key)"><a href="#">{{item.label}}</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
            ...
            <section>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4 mb-5" v-for="(item, index) in objectList">
                        <div class="item">
                            <h4 class="item__title"><a :href="`/service/view/${item.id}`">{{item.title}}</a></h4>
                            ...
                            <a :href="`/service/view/${item.id}`" class="item__overlay-link">Заголовок второго уровня</a>
                        </div>
                    </div>
                    <!-- end col -->
                </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            getServiceTypeOptions: function () {
                this.axios.post('/getServiceTypeList', {}).then(response => {
                    this.serviceTypeOptions = response.data;
                    this.serviceTypeOptions.unshift({
                        class : "current",
                        key : 0,
                        label : "Не выбрано"
                    })
            ...
            changeServiceTypeKey: function (key) {
                this.type = key;
                var label = '';
                this.serviceTypeOptions.forEach(function(option){
                    if(option.key === key){
                        option.class = 'current';
                        label = option.label;
                    }else{
